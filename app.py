...
# ==========================================
    ...
    for idx, tab in enumerate(tabs):
        with tab:
            ...
            # --- [수정 완료] 사용자가 지정한 마감일 기준으로 D-day 실시간 계산 ---
            today = date.today()
            if cat in st.session_state.cat_dates:
                target_date = st.session_state.cat_dates[cat]
                d_day = (target_date - today).days # 설정값 날짜 - 오늘 날짜 차이 계산
                ...
                # 상단 알림창 (항상 표시되도록 조건 제한 해제)
                st.markdown(f"""
                    <div class="alert-box">
                        <h3 style="margin-top:0;">⚠️ D-day 알림!</h3>
            ...
            if show_preview:
                ...
                with col_p2:
                    # 설정된 마감일을 달력으로 유저가 변경하면 알림창의 D-day 텍스트가 즉시 연동되어 실시간 반영됨
